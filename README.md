...
- **録音 → 文字起こし → 要約**: 録音開始で3秒カウントダウン → 録音（音声レベルバー・経過時間・長時間警告つき、停止はスライド操作で誤タップ防止）→ 停止 → 文字起こしを手動編集 → Claude で構造化要約
...
- **直接録音**: ホームの「録音を始める」からクライアントを選ばず即録音。要約後の保存時に既存クライアントを選ぶか新規登録する
...
- **クライアント管理**: 追加（グループ選択つき）・リネーム・削除、グループへの分類（折りたたみUI）
