...
- **ホーム画面ウィジェット**: 「本日の予定」＋「今日までのToDo」を表示するAndroidホーム画面ウィジェット（全ユーザー無料）。タップでアプリを開く
...
- **ToDo ボード**（`ui/client/FollowupListScreen`）: 「ToDo」「完了」の2タブ。**ToDo タブに期限フィルタ**（すべて／期限が3日以内／期限切れ。ホームの「すべて表示」から遷移した際はその条件で自動的に絞り込まれる）＋並び替え・追加。各 ToDo 行のベルアイコンから**通知日時を指定**でき、指定した日時に端末通知が届く（旧「スヌーズ」機能は2026-09-16に廃止し、この通知予約方式に置き換え済み）。**完了タブに「すべて削除」**（確認ダイアログあり、全クライアント横断で一括削除）
...
| DB | Room 2.8.4（KSPでコード生成）。現在 version 31 |
| バックグラウンド | WorkManager（リマインド通知・ToDo通知の周期/単発実行） |
...
| 分析・クラッシュ検知 | Firebase Analytics / Crashlytics（2026-09-23〜。`google-services.json` 未配置なら自動的に無効化、設定画面でユーザーが送信ON/OFF切替可） |
| 広告 | AdMob（`play-services-ads` 25.4.0） |
| 課金 | Google Play Billing（`com.android.billingclient:billing` 8.3.0） |
| 不正対策 | Play Integrity API（Standard。`server/` 側で decode。設定完了・`audit` モードで検証中） |
...
- Google Play Console でクローズドテスト提出作業中（`versionCode 12` / `versionName 0.2.0`。実際に配信されていた直近版は `versionCode 6` / `0.1.5` で、そこからの大型アップデート）。Play Integrity API のフェーズ2設定を完了させ `audit` モードで検証中、Firebase Analytics/Crashlytics を導入、Play Billing Library を8.3.0に更新済み（詳細は `CLAUDE.md`「現在の開発状況」を参照）
- 未完了: Google Play Billing の有効化（基盤は実装済み、Play Console 商品登録＋`PRO_GATING_ENABLED=true`）、Play Integrity を `audit`→`true` へ切替（クローズドテスト公開後に実機で検証してから）、Pro 機能（F4 根拠リンク・F6 ヒアリング分析）、ネイビー配色の残り（録音3画面・ダッシュボード2分割・アバター等、CLAUDE.md 参照）
