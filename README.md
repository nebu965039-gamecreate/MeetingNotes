...
- **無料枠クレジット制**: 月5回まで無料で要約可能(`CreditPolicy.MONTHLY_FREE_CREDITS`)。リワード広告視聴で追加クレジット獲得。要約失敗時は自動返却。1回の文字起こしは約20,000字(60〜80分)まで
- **広告**: AdMob。バナー(クライアント一覧・商談アーカイブ下部/アダプティブ、商談詳細画面中央/300x250)、インタースティシャル(要約リクエスト直後=待ち時間に表示)、リワード(クレジット切れ時)。既定はGoogle公式テスト広告。`local.properties` の `ADMOB_USE_PRODUCTION_ADS=true` で本番の広告ユニットに切替(本番/オープンテスト用ビルドのみ)
