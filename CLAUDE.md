...
- **フォローアップ下書きを要約時に生成(2026-09-07)**: トークン節約のため、`/summarize` の tool スキーマに `followupDraft`(丁寧体1本)を追加し要約と同時に生成。`MeetingSummary.followupDraft` / `SummaryDto` / `toDomain` / `saveMeeting` 対応、**`MeetingEntity.followupDraft`(DB version 10 + `MIGRATION_9_10`、`10.json`、`MigrationTest` 9→10)**。`MeetingDetailScreen` は `meeting.followupDraft` があれば `viewModel.showStoredFollowup()` で即表示(`FollowupState.Ready(stored=true)`、トーン切替なし・API 呼び出しなし)。旧データ(null)は従来どおり `generateFollowup` で `/followup` を都度呼ぶフォールバック。Worker `MAX_TOKENS` 2000→2400。**Worker 再デプロイ必要**
- **アーカイブ `MeetingRow` の見た目(2026-09-07)**: タイトルは `surface`(白系)の帯 + 右端に `DealPhaseChip(outlined = true)`(枠のみ・コンパクト、タップで変更)。要約プレビューは `conciseSummary()`(先頭1文・最長60字)で見切れ回避。`DealPhaseChip` に `outlined` パラメータ追加
...
- **【要実機検証】PCスピーカーから出る相手の声が認識されない / 認識ごとの効果音**: `SpeechRecognizer` が自分でマイクを開くと近接音声向け処理(ノイズ抑制・AGC)が入り、離れた相手の声を削る。加えて多くの端末はマイクを開くたび効果音を鳴らす。2026-09-06 の `EXTRA_AUDIO_SOURCE` 付与(extra は無視されがち)は **revert 済み**。2026-09-07、`TranscriptionManager.MicAudioFeeder` を新設: 録音中ずっと自前の `AudioRecord`(`UNPROCESSED`→`MIC`、`VOICE_RECOGNITION` は使わない、16kHz mono)を保持し、認識セッションごとに `ParcelFileDescriptor.createPipe()` の書き込み端を差し替えて `EXTRA_AUDIO_SOURCE` で recognizer に供給する。認識器がマイクを開かない=遠い声が残りやすく効果音も出にくいのが狙い。開始失敗・連続エラー(`consecutiveErrors>=2` かつ未成功)・無音セッション連続(`emptyFeederSessions>=3`)なら `disableAudioFeeder()` で通常経路(recognizer が直接マイク)へフォールバック。音声レベル表示はフィーダー使用時は AudioRecord 振幅から算出。**実機での効果(遠い声を拾えるか/効果音が消えるか/認識精度)は未検証**。`STREAM_MUSIC/SYSTEM/NOTIFICATION` のミュートは belt-and-suspenders で継続。Zoom/Teams の文字起こし連携は不可(サードパーティにライブ字幕を公開していない)。`MediaProjection` の再生音キャプチャも VoIP 音声(`USAGE_VOICE_COMMUNICATION`)は仕様上キャプチャ不可
