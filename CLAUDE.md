...
- **フォローアップ下書きは要約時のみ生成(2026-09-07)**: 個別生成の導線は**廃止**。`/summarize` の tool スキーマに `followupDraft`(丁寧体1本、ToDo を「次のアクションのお願い」に織り込む)を追加し要約と同時に生成。`MeetingSummary.followupDraft` / `SummaryDto` / `toDomain` / `saveMeeting` 対応、**`MeetingEntity.followupDraft`(DB version 10 + `MIGRATION_9_10`、`10.json`、`MigrationTest` 9→10)**。`FollowupState` は `Idle`/`Ready(text)`/`Empty` の3状態のみ(生成・トーン切替・エラー再試行なし)。`MeetingDetailScreen` の「フォローアップの下書きを見る」→ `viewModel.openFollowup()` で `followupDraft` を表示、無ければ `Empty`(「要約時に自動生成されます」の説明のみ、生成ボタンなし)。**削除したもの**: Worker `/followup` エンドポイント・`FOLLOWUP_SYSTEM_*`・`handleFollowup`、`AnthropicClient.generateFollowup`、`FollowupRequest`、`MeetingDetailViewModel.generateFollowup`/`buildPlainTextSummary`。Worker `MAX_TOKENS` 2000→2400。**Worker 再デプロイ必要**
- **フェーズタグの見た目統一(2026-09-07)**: `DealPhaseChip` は「枠あり + 塗りつぶし」の1スタイルに固定(`RoundedCornerShape(6dp)`、`labelMedium`)。予定(`UpcomingBoard`/`ScheduleScreen`)・商談詳細・アーカイブ・フォロー一覧すべて同じ見た目。一時追加した `outlined` パラメータは撤去。**配色は `ui/theme/PhaseTagColors.kt`**: アプリ全体が M3 紫系で埋もれるため、各フェーズに別系統色(初回接触=スレート/ヒアリング=琥珀/提案=青/見積提示=ティール/検討中=オレンジ/成約=緑/保留=温グレー/失注=ローズ)を割り当て。意味を持つ状態色のためテーマ非追従(淡い塗り+濃い文字、`PhaseTrackerColors` と同じ方針)
...
- **【未解決】PCスピーカーから出る相手の声が認識されない / 認識ごとの効果音**: `SpeechRecognizer` が自分でマイクを開くと近接音声向け処理(ノイズ抑制・AGC)が入り、離れた相手の声を削る。加えて多くの端末はマイクを開くたび効果音を鳴らす。試した対策と結果:
  ...
  - 2026-09-07、`3299e34` ベースに (a) 録音中は `STREAM_MUSIC` の音量そのものを 0 に落として `stop()` で復元(`adjustStreamVolume(MUTE)` は SYSTEM/NOTIFICATION には DND 権限が無いと効かないため) (b) `EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS` 等に 20秒を指定してセッションが沈黙で切れる頻度を下げる(=効果音・再開回数を減らす。端末が無視する場合あり) (c) `onPartialResults` を live 表示に反映(セッションが長引いてもテキストが流れる)。**効果音を端末レベルで完全に消すには DND 権限が必要で未実装**
