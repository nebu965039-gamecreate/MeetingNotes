...
- **フォルダ/グループ作成の UI**: クライアント一覧・アーカイブ(`ClientDetailScreen`)ともに **`Icons.Filled.CreateNewFolder`(青 tint)**。片方の見た目を変えたらもう片方も揃える。なお `ClientDetailScreen` はクライアント名が長いと圧迫されるため、`topBar` を `Column { TopAppBar(タイトル=クライアント名, maxLines=2) + Surface(primaryContainer){ Row(Arrangement.End){ 検索 / CreateNewFolder / ⋮ } } }` の2段構成にしてある(操作アイコンは2段目の右詰め)。⋮ メニュー = 前回のおさらい / クライアント情報(→ `ClientInfoScreen`) / クライアントを削除
...
- **ホーム画面(2026-09-05〜、`ui/home/HomeScreen.kt` + `HomeViewModel.kt`。2026-09-08 に下部ナビ導入で再構成)**: 旧来は `ClientListScreen` がアプリ起動直後だったが `HomeScreen` に置き換え。**構成(2026-09-08)**: ヘッダー(日付+挨拶+**通知ベル**〈未読で `Badge`〉+設定+ヘルプ)→ **ダッシュボード**(`HomeDashboardCard`、最上部固定)→ 下書き復元カード(あれば)→ やること（期限あり）(`DueTodoBoard`、非空時)→ 直近の予定(`UpcomingBoard`)→ ToDo(`FollowupBoard`)→ **進行中のフェーズ**(横棒ファネル)。**アクションタイル(2×2)は廃止**(録音=FAB、クライアント一覧/予定表=下部ナビ、通知=ヘッダーベル)。`onOpenSchedule`/`onOpenFollowupList` は下部ナビのタブ切替(`switchTab`)に配線。
