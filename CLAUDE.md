...
- **ホーム画面(2026-09-05〜、NavHost の start destination)**: 旧来は `ClientListScreen`(クライアント一覧)がアプリ起動直後の画面だったが、`ui/home/HomeScreen.kt`(+`HomeViewModel.kt`)に置き換え。`ClientListScreen` は「クライアント一覧」タイルから遷移するサブ画面になり、`onMeetingSelected`/`onRecoverDraft`/`onShowNotifications`/`onHelp` は撤去(`onClientSelected`/`onBack` のみ)。ホーム画面の構成: ヘッダー(日付+挨拶+ヘルプ)→ 下書き復元カード(あれば)→ アクションタイル(録音を始める/クライアント一覧/予定表/通知、いずれも `surfaceContainerLow` の枠で囲みグループを明確化)→ **進行中のフェーズ**(ステップ・トラッカー。ヒアリング/提案/見積提示/検討中のみ集計、初回接触・保留・成約・失注は対象外。色は `ui/theme/PhaseTrackerColors.kt` で状態色として明暗共通固定)→ **要フォロー**(`FollowupBoard`、`surfaceContainerLow` 枠+内部 `tertiaryContainer` カード、`heightIn(max=152.dp)+verticalScroll` で常時3件分の高さ・最大10件まで内部スクロール、11件目以降がある場合だけ末尾に「すべて表示」)→ **次回の予定**(`UpcomingBoard`、`surfaceContainerLow` 枠+`secondaryContainer` カード、プレビュー3件+ヘッダーに「すべて表示」)。「もっと見る」表記は全て「すべて表示」に統一。フェーズタグの表示は既存の `ui/common/DealPhaseChip.kt` を再利用(新規コンポーネント化しない)。日付ラベル(本日/明日/M-d)は `ui/common/DateLabels.kt` の `relativeDateTimeLabel` に集約しホーム/予定表で共有。「要フォロー」「予定表」それぞれの全件ページは `ui/client/FollowupListScreen.kt` と `ui/schedule/ScheduleScreen.kt`(いずれも専用の軽量 ViewModel、`FollowupRules.compute`/`UpcomingRules.compute` をキャップなしで使う)。「録音を始める」タイルはクライアント選択が必須なため暫定でクライアント一覧へ遷移(専用のクライアント選択シートは未実装)
