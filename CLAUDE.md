...
- **フォローアップメール ToDo の自動起票(2026-09-08、DB v15)**: `MeetingRepository.saveMeeting` が要約成功時に、要約由来の ToDo に加えて **「お礼・フォローアップのメールを送る」ToDo を必ず1件**起票する(`TodoEntity.isFollowupEmail = true`、`dueDate` = 録音日の翌日、`assignee` = 「自分」。フェーズ問わず・既存商談へのさかのぼりはしない)。これでクライアントの ToDo リスト/`TodoCountBadge`/ホーム「やること(期限あり)」/期限リマインダーに要約 ToDo と並んで出る。**F1「ToDo」ボードとの一元化**: このフラグの ToDo を完了/未完了にすると `meetings.followedUpAt` が同期される(`MeetingRepository.setTodoDone` が `todoDao.getById` で判定 → `updateFollowedUpAt`)。逆にボードの「完了」/「ToDoに戻す」(`markMeetingFollowedUp`/`clearMeetingFollowedUp`)も `todoDao.followupEmailTodoId` 経由でメール ToDo の `isDone` を反転する。旧データ(メール ToDo 無し)は従来どおり `followedUpAt` のみで動く。`FollowupRules`・ボードの UI シグネチャは不変(`followedUpAt` を読むだけ)
