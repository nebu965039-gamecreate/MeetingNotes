...
- **配色**: M3 標準の紫パレット(`lightColorScheme()`/`darkColorScheme()` とも引数なしの baseline。カスタム色は使わない)。**2026-09-06〜ダークテーマ対応**: `MainActivity.kt` が `MeetingNotesApp.themeModeState`(`ui/theme/ThemeMode`: LIGHT/DARK/SYSTEM、`ui/theme/ThemePrefs.kt` で永続化)を読み `darkColorScheme()`/`lightColorScheme()` を出し分けて `MaterialTheme(colorScheme = ...)` に渡す。設定は `ui/settings/SettingsScreen.kt`(「背景(テーマ)」行→ダイアログで3択)。`app.setThemeMode()` が Compose の `MutableState` を書き換えるため全画面へ即時反映される。例外として「作成」系アクション(クライアント追加 FAB・フォルダ/グループ作成アイコン)は `ui/theme/CreateActionColors.kt` の `CreateActionBlue`(#1565C0)、ホームのダッシュボード(ドーナツ)・「進行中のフェーズ」ファネルは `ui/theme/PhaseChartColors.kt`(`PhaseTagColors` 同系・ライト/ダーク明度入替) — これらは M3 パレットに従わない専用色。ヘルプ画面(`ui/help/HelpScreen.kt`)はカード + アイコン + セクション別アクセント色(紫/青/緑/スレート、`Accent*` 定数)のデザイン(ダークテーマでの見え方は未検証)
...
- **クライアント配下のプロジェクト(任意、2026-09-09、DB v16)**: 1クライアントに 0..N 個。`client_projects` テーブル + `meetings.projectId`(FK なし)。`ClientProjectDao` / `MeetingRepository.observeClientProjects` / `addClientProject` / `renameClientProject` / `deleteClientProject`(先に `clearProject` で商談の projectId を null 化)/ `setMeetingProject`。
  - `ClientDetailViewModel`: `projects` / `projectFilter`(`null`=すべて / `NO_PROJECT`(-1L)=プロジェクトなし / それ以外=ID)。`sortedMeetings`・`searchResults` は `applyProjectFilter` 適用後。CRUD メソッドあり。
  - UI(アーカイブタブ): `projects` が1件以上のとき先頭に `LabeledDropdownField`(すべて/なし/各プロジェクト)で絞り込み。⋮ メニュー「プロジェクトを管理」→ `ManageProjectsDialog`(追加/改名(`TextInputDialog`)/削除(`ConfirmDialog`))。`MeetingRow` の ⋮ に「プロジェクトを変更」→ `ProjectPickerDialog`(なし + 各プロジェクトのラジオ)。行の日付の右に `・📁 プロジェクト名` を表示。
  - ToDo タブのプロジェクト絞り込みは未対応(アーカイブのみ)。
