...
- `app/src/main/java/com/meetingnotes/data/local/MeetingNotesDatabase.kt` — Room DB定義。現在 version = 16(… v13 = `client_contacts` テーブル(担当者)、v14 = `client_contacts.email/phone`、v15 = `todos.isFollowupEmail`(自動起票フォローアップメール ToDo の識別)、v16 = `client_projects` テーブル(クライアント配下の任意プロジェクト)+ `meetings.projectId`(**FK なしの単純カラム** — SQLite の ALTER で FK は張れないため。プロジェクト削除時は `MeetingRepository.deleteClientProject` が先に `meetingDao.clearProject` で null に戻す)。`feature/solo-crm` → `main` マージ済み)
