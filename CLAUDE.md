...
- `app/src/main/java/com/meetingnotes/data/local/MeetingNotesDatabase.kt` — Room DB定義。現在 version = 18(… v15 = `todos.isFollowupEmail`、v16 = `client_projects` テーブル + `meetings.projectId`(FK なし。削除時 `clearProject`)、v17 = `schedules` テーブル(独立した「予定」。1クライアントに複数)、v18 = `client_projects` に `phase`/`currency`/`estimatedAmount`/`wonAmount`/`wonAt`(案件の金額・状態)。`feature/solo-crm` → `main` マージ済み)
...
- **Tier 1 CRM 補強(2026-09-08)**: 他AIの機能提案(`docs` 外)との差分埋め。**DB version 12 + `MIGRATION_11_12`**(`12.json`、`MigrationTest` 11→12)。
  ...
  - **ホーム/全件ページの「ToDo」ボード**(`FollowupBoard`/`FollowupListScreen`): 行タップを**クライアント詳細**へ変更(旧: 商談詳細)。クライアント名の右に `TodoCountBadge`(未完了 ToDo 件数)。件数は `TodoDao.observeOpenTodoCountByClient`(`todos`⨯`meetings` JOIN, `isDone=0`, `GROUP BY m.clientId`)→ `MeetingRepository.observeOpenTodoCountByClient(): Flow<Map<Long,Int>>` → `FollowupRules.compute(…, openTodoCountByClient=)`。`FollowupBoard` の「ToDo」= 各クライアント最新商談で `followedUpAt==null` かつ 成約/失注でない かつ 最終商談から30日以内(`DueTodoBoard` の「やること(期限あり)」は個々の期限付きタスク単位で別物)
