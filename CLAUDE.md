...
- **フェーズタグの見た目統一(2026-09-07)**: `DealPhaseChip` は「枠あり + 塗りつぶし」の1スタイルに固定(`RoundedCornerShape(6dp)`、`secondaryContainer` + `outline` 枠、`labelMedium`)。予定(`UpcomingBoard`/`ScheduleScreen`)・商談詳細・アーカイブすべて同じ見た目。一時追加した `outlined` パラメータは撤去
