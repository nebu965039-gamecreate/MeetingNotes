...
- `app/src/main/java/com/meetingnotes/data/local/MeetingNotesDatabase.kt` — Room DB定義。現在 version = 17(… v15 = `todos.isFollowupEmail`、v16 = `client_projects` テーブル + `meetings.projectId`(FK なし。削除時 `clearProject`)、v17 = `schedules` テーブル(独立した「予定」。1クライアントに複数)。`feature/solo-crm` → `main` マージ済み)
...
- **クライアント配下のプロジェクト(任意、2026-09-09、DB v16)**: 1クライアントに 0..N 個。`client_projects` テーブル + `meetings.projectId`(FK なし)。`ClientProjectDao` / `MeetingRepository.observeClientProjects` / `addClientProject` / `renameClientProject` / `deleteClientProject`(先に `clearProject` で商談の projectId を null 化)/ `setMeetingProject`。
  - `ClientDetailViewModel`: `projects` / `projectFilter`(`null`=すべて / `NO_PROJECT`(-1L)=プロジェクトなし / それ以外=ID)。`sortedMeetings`・`searchResults`(`applyProjectFilter`)に加え **`openTodos`・`doneTodos` も `_projectFilter` で絞り込む**(`meetingProjectById` で商談→プロジェクトを引き当て、`matchesProject` 共通判定)。CRUD メソッドあり。
  - UI: `projects` が1件以上のとき **タブ直下**に `ProjectFilterBar`(コンパクトな `TextButton` + `DropdownMenu`。すべて/なし/各プロジェクト)を表示し **アーカイブ・ToDo 両タブに効く**(情報タブでは非表示)。⋮ メニュー「プロジェクトを管理」→ `ManageProjectsDialog`(追加/改名(`TextInputDialog`)/削除(`ConfirmDialog`))。`MeetingRow` の ⋮ に「プロジェクトを変更」→ `ProjectPickerDialog`。行の日付の右に `・📁 プロジェクト名`。
