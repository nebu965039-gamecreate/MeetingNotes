...
- `app/src/main/java/com/meetingnotes/data/local/MeetingNotesDatabase.kt` — Room DB定義。現在 version = 19(… v15 = `todos.isFollowupEmail`、v16 = `client_projects` テーブル + `meetings.projectId`(FK なし。削除時 `clearProject`)、v17 = `schedules` テーブル(独立した「予定」。1クライアントに複数)、v18 = `client_projects` に `phase`/`currency`/`estimatedAmount`/`wonAmount`/`wonAt`(案件の金額・状態)、v19 = `todos` 再構築(`meetingId` を nullable に・`clientId` NOT NULL + clients へ FK CASCADE を追加。手動 ToDo 対応。既存行の `clientId` は `meetings.clientId` から補完)、v20 = `client_projects.lostReason`(失注理由)。`feature/solo-crm` → `main` マージ済み)
...
- **配色**: M3 標準の紫パレット(`lightColorScheme()`/`darkColorScheme()` とも引数なしの baseline。カスタム `ColorScheme` は無い)。**2026-09-09〜「紫を使う場所を絞る」方針(案A+B)**: (A) 一覧系画面(`ClientListScreen`/`ScheduleScreen`/`NotificationScreen`/`FollowupListScreen`/`ClientDetailScreen`)の TopAppBar は `primaryContainer` 配色を**やめて既定の `surface`** + 下に `HorizontalDivider(outlineVariant)`。タイトル左アイコンだけ `primary` tint。(B) セクションカードの塗り(`UpcomingBoard`/`FollowupBoard`/`FollowupListScreen` の内側パネル、`MeetingRow`/`ClientRow`/`ScheduleRow`/通知カード、下書き復元カード、`BriefingScreen` の hero)は `secondary/tertiaryContainer` → **`surfaceContainer`/`surfaceContainerHigh`(中立)**。区切り線は `onXxxContainer.copy(alpha)` → `outlineVariant`。**紫(`primary`)が残るのは**: 主要ボタン / リンク(下線) / 下部ナビの選択中ピル / `DealPhaseChip` 以外のチップ / ダッシュボードのアクセント / タイトルアイコン。`HelpScreen`・`RecordingScreen` は対象外(独自デザイン / 機能画面)。**2026-09-06〜ダークテーマ対応**: `MainActivity.kt` が `MeetingNotesApp.themeModeState`(`ui/theme/ThemeMode`: LIGHT/DARK/SYSTEM、`ui/theme/ThemePrefs.kt` で永続化)を読み `darkColorScheme()`/`lightColorScheme()` を出し分けて `MaterialTheme(colorScheme = ...)` に渡す。設定は `ui/settings/SettingsScreen.kt`(「背景(テーマ)」行→ダイアログで3択)。`app.setThemeMode()` が Compose の `MutableState` を書き換えるため全画面へ即時反映される。例外として「作成/録音」系アクション(クライアント追加 FAB・フォルダ/グループ作成アイコン・**ホームの録音 FAB**)は `ui/theme/CreateActionColors.kt` の `CreateActionBlue`(#1565C0)、ホームのダッシュボード(ドーナツ)は `ui/theme/PhaseChartColors.kt`(`PhaseTagColors` 同系・ライト/ダーク明度入替) — これらは M3 パレットに従わない専用色。ヘルプ画面(`ui/help/HelpScreen.kt`)はカード + アイコン + セクション別アクセント色(紫/青/緑/スレート、`Accent*` 定数)のデザイン(ダークテーマでの見え方は未検証)
- **フォルダ/グループ作成の UI**: クライアント一覧・`ClientDetailScreen` アーカイブタブともに **`Icons.Filled.CreateNewFolder`(青 tint)**。片方の見た目を変えたらもう片方も揃える。
- **`ClientDetailScreen` は3タブ構成(2026-09-09、`PrimaryTabRow`)**: `topBar` = `Column { TopAppBar(タイトル=クライアント名 maxLines=2, actions=⋮) + PrimaryTabRow }`。⋮ メニュー = 前回のおさらい / クライアントを削除。
  - **アーカイブ**タブ: 商談一覧。ヘッダー行に「アーカイブ」+ 検索 / `CreateNewFolder`(青) / 並び替え をまとめる。検索・フォルダ折りたたみ・`MeetingRow`(`DealPhaseChip`)は従来どおり。
  - **ToDo**タブ(`TodoTab`): 「ToDo」「完了」の2サブタブ(`PrimaryTabRow`)。ToDo = `ClientDetailViewModel.openTodos`(`Checkbox` で完了、行タップで商談詳細)、完了 = `.doneTodos`(id 降順、`Checkbox` を外すと `reopenTodo`、打消し線)。タブラベルに未完了件数。
  - **情報**タブ: `ClientInfoContent`(`ClientInfoScreen.kt`、旧スタンドアロン `ClientInfoScreen` から抽出)。名前/グループ/現在のステータス(`DealPhaseChip`)/メール/電話/担当者/備考の閲覧 + 「情報を編集」→ `ClientEditScreen`(`Routes.CLIENT_EDIT`)。旧 `Routes.CLIENT_INFO` は廃止。
  - `bottomBar` = `BannerAdView` + 「録音開始」ボタン(全タブ共通。2回目以降は `onShowBriefing`。判定は `viewModel.hasAnyMeeting` = プロジェクト絞り込みに影響されない生の件数)。`ClientDetailScreen` の引数は `onOpenClientInfo` → `onEditClient` に変更。
- **クライアント配下のプロジェクト(= 案件、任意、2026-09-09、DB v16 / 金額は v18)**: 1クライアントに 0..N 個。`client_projects` テーブル + `meetings.projectId`(FK なし)。`ClientProjectDao` / `MeetingRepository.observeClientProjects` / `observeAllProjects` / `addClientProject` / `updateClientProject` / `deleteClientProject`(先に `clearProject` で商談の projectId を null 化)/ `setMeetingProject`。
  ...
  - UI: `projects` が1件以上のとき **タブ直下**に `ProjectFilterBar`(コンパクトな `TextButton` + `DropdownMenu`。すべて/なし/各プロジェクト)を表示し **アーカイブ・ToDo 両タブに効く**(情報タブでは非表示)。⋮ メニュー「プロジェクトを管理」→ `ManageProjectsDialog`(一覧に名前+`DealPhaseChip`+金額サマリ、行タップで `ProjectFormDialog` 編集 / 「案件を追加」/ 削除アイコン→`ConfirmDialog`)。`ProjectFormDialog` = 案件名 + 進捗/通貨の `LabeledDropdownField` + 見積額/成約額の数値入力 + `phase==WON` のとき成約日ボタン(`NextMeetingDateTimeDialog` 再利用)。`MeetingRow` の ⋮ に「プロジェクトを変更」→ `ProjectPickerDialog`。行の日付の右に `・📁 プロジェクト名`。
  - **情報タブ(`ClientInfoContent`)**: 案件が1件以上のとき「案件」セクション(各案件の名前+`DealPhaseChip`+見積/成約額、通貨ごとの合計)。`ClientInfoViewModel.projects`。
  - **売上・実績ビュー(Pro、2026-09-09、`ui/sales/`)**: `Routes.SALES`。ホームの `HomeDashboardCard` 下部「売上・実績を見る ›」(`onOpenSales`)から遷移。`SalesRules`(純粋関数の `object`、`SalesRulesTest` 対象)が `observeAllProjects()` を **通貨ごとの `SalesCurrencyReport`** に集計: 直近12ヶ月の月次成約額 / 期間内(今月・今年・全期間、`SalesPeriod`)の成約額・件数・平均単価 / 全期間の成約率(金額 = 成約額 ÷ (成約額+失注見積額)、件数 = 成約数 ÷ (成約数+失注数))/ 失注理由の内訳(`lostReasonBreakdown`)/ 進行中案件のフェーズ別パイプライン。`SalesViewModel` は `_period` と `observeAllProjects()` を combine。`SalesScreen` = TopAppBar + `SingleChoiceSegmentedButtonRow`(期間)+ 通貨ごとの Card(`Canvas` 手描き月次棒グラフ + サマリー + フェーズ別パイプラインの横棒)。**`ProAccess.shouldLock`(現状 false)が true のときは `LockedContent` + `ProPaywallDialog` を出すだけ**(Billing 実装後に有効化)。棒グラフ色は `PhaseChartColors.of(WON, darkTheme)`。
...
- **下部ナビ + 録音FAB(2026-09-08、`ui/home/MainTabs.kt`)**: `Routes.MAIN` = `MainTabsShell`(NavHost の start destination)。内側 `NavHost`(`tabNav`)で **4タブ**(ホーム/クライアント/予定表/ToDo)を保持し、詳細画面(クライアント詳細・商談詳細・録音・設定・ヘルプ・通知など)は外側 `navController` へ委譲する。`Scaffold(contentWindowInsets = WindowInsets(0))` の `bottomBar` = `Column { BannerAdView(); NavigationBar {…} }`(**共通バナー広告はここに1つだけ**。各タブ画面から自前の `bottomBar = BannerAdView` は撤去)。**録音ボタンは `NavigationBar` の中央**(2026-09-09、旧: 右下 FAB): 4タブ + 中央の空きスロット(`Spacer(Modifier.weight(1f))`)の上に、丸い録音ボタン(`Surface(CircleShape, `primary`)` を `Box` overlay)を重ねる。`ui/common/PulsingHalo`(静的リング + 控えめな波紋1本。OS のアニメ OFF なら静的のみ)で囲む。**全タブから押せる**(`onStartRecording` = クライアント未指定録音)。ToDo タブに `Badge`(`TodoDao.observeOpenTodoTotal`)。`NavigationBarItem` は `RowScope` 拡張 `NavTab`(ラベル 10sp / `maxLines=1` / `softWrap=false` で「クライアント」の改行を防ぐ)。カウントダウン画面(`RecordingScreen.CountdownContent`)も `PulsingHalo`(やや強め)+ `AnimatedContent` で数字をティックごとにポップ。`ClientListScreen`/`ScheduleScreen`/`FollowupListScreen` は **戻る矢印を撤去**(タブなので)、`onBack` 引数も削除、`contentWindowInsets = WindowInsets(0)`(ヘッダー配色は 2026-09-09 の配色見直しで `surface` + 罫線に。上記「配色」項参照)。旧 `Routes.HOME`/`CLIENT_LIST`/`SCHEDULE`/`FOLLOWUP_LIST` は廃止。`onClientDeleted`・`ResultScreen.onSaved`(直接録音フロー)は `popBackStack(Routes.MAIN)` へ。
- **ホーム画面(2026-09-05〜、`ui/home/HomeScreen.kt` + `HomeViewModel.kt`。2026-09-08 に下部ナビ導入で再構成)**: 旧来は `ClientListScreen` がアプリ起動直後だったが `HomeScreen` に置き換え。**構成(2026-09-08)**: ヘッダー(日付+挨拶+**通知ベル**〈未読で `Badge`〉+設定+ヘルプ)→ **ダッシュボード**(`HomeDashboardCard`、最上部固定)→ 下書き復元カード(あれば)→ やること（期限あり）(`DueTodoBoard`、非空時)→ 直近の予定(`UpcomingBoard`)→ ToDo(`FollowupBoard`)。**アクションタイル(2×2)は廃止**(録音=FAB、クライアント一覧/予定表=下部ナビ、通知=ヘッダーベル)。**「進行中のフェーズ」セクションは削除(2026-09-09)** — ダッシュボードのドーナツと重複するため。`HomeViewModel.phaseCounts` 公開フローも撤去(`phaseCountsOf` はダッシュボードで継続使用)。`onOpenSchedule`/`onOpenFollowupList` は下部ナビのタブ切替(`switchTab`)に配線。
  ...
  - 共通事項: リンク文字は `TextDecoration.Underline`(件数表示と区別)。日付ラベル(本日/明日/M-d)は `ui/common/DateLabels.kt` の `relativeDateTimeLabel`。`DueTodoBoard` の内側は `surface`(白)の角丸パネル + 丸チェックアイコン。`UpcomingBoard` の内側は `secondaryContainer` パネル + 日時ラベル(上)/クライアント名(下)+ `DealPhaseChip` + `ChevronRight` + 区切り線、プレビュー3件。
...
- **予定は独立エンティティ `schedules`(2026-09-09、DB v17)**: 旧来は「予定 = `MeetingEntity.nextMeetingDate`」で1クライアント1件・上書きだったが、`ScheduleEntity`(`clientId` FK CASCADE / `sourceMeetingId: Long?` / `startAtMillis` / `hasTime` / `title` / `note` / `participants` / `phase`)に分離。1クライアントに複数持てる。
  ...
  - **予定表(`ScheduleScreen`)**: 月カレンダー(`MonthCalendar`、予定日 + ToDo 期限日に丸)+ 一覧(日付タップで絞り込み。未選択時はこれからの予定、選択時はその日=過去も表示)。FAB / 行の ⋮「編集」→ `ScheduleFormDialog`(クライアント`LabeledDropdownField`(全クライアント)/ 日時 `NextMeetingDateTimeDialog` / 内容・タイトル / 参加者 / メモ / 進捗ラベル `LabeledDropdownField<DealPhase?>`)。⋮「削除」= `ConfirmDialog`(AI 由来なら nextMeetingDate も戻る旨を表示)。`ScheduleRow` は タイトル + フェーズchip + 「クライアント名 ・ 参加者」 + メモ。
...
- **フォローアップメール ToDo の自動起票(2026-09-08、DB v15)**: `MeetingRepository.saveMeeting` が要約成功時に、要約由来の ToDo に加えて **「お礼・フォローアップのメールを送る」ToDo を必ず1件**起票する(`TodoEntity.isFollowupEmail = true`、`dueDate` = 録音日の翌日、`assignee` = 「自分」。フェーズ問わず・既存商談へのさかのぼりはしない)。これでクライアントの ToDo リスト/`TodoCountBadge`/ホーム「やること(期限あり)」/期限リマインダーに要約 ToDo と並んで出る。
  - **ホーム「ToDo」ボードは ToDo 駆動**(2026-09-08 改): `FollowupRules` は「未完了 ToDo が1件以上あるクライアント」を出すだけ。行内「完了」ボタンは廃止(ホーム `FollowupBoard`・全件 `FollowupListScreen` の ToDo タブ両方)。クライアント画面/商談詳細で ToDo を全部チェックするとボードから自動的に消える。ヘッダー件数はクライアント数でなく未完了 ToDo 合計。
