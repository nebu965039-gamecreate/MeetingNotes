...
- `app/src/main/java/com/meetingnotes/data/local/MeetingNotesDatabase.kt` — Room DB定義。現在 version = 15(… v11 = `meetings.meetingType` + `user_credits` オンライン回数、v12 = `todos.dueDate` + `clients.email/phone`、v13 = `client_contacts` テーブル(担当者)、v14 = `client_contacts.email/phone`、v15 = `todos.isFollowupEmail`(要約完了時に自動起票するフォローアップメール ToDo の識別フラグ)。`feature/solo-crm` ブランチ)
...
- **`ClientDetailScreen` は3タブ構成(2026-09-09、`PrimaryTabRow`)**: `topBar` = `Column { TopAppBar(タイトル=クライアント名 maxLines=2, actions=⋮) + PrimaryTabRow }`。⋮ メニュー = 前回のおさらい / クライアントを削除。
  ...
  - `bottomBar` = `BannerAdView` + 「録音開始」ボタン(全タブ共通。2回目以降は `onShowBriefing`)。`ClientDetailScreen` の引数は `onOpenClientInfo` → `onEditClient` に変更。
