...
- **フォローアップ下書きを要約時に生成(2026-09-07)**: トークン節約のため、`/summarize` の tool スキーマに `followupDraft`(丁寧体1本)を追加し要約と同時に生成。`MeetingSummary.followupDraft` / `SummaryDto` / `toDomain` / `saveMeeting` 対応、**`MeetingEntity.followupDraft`(DB version 10 + `MIGRATION_9_10`、`10.json`、`MigrationTest` 9→10)**。`MeetingDetailScreen` は `viewModel.openFollowup()`: 保存済み下書きがあれば即表示(`FollowupState.Ready(stored=true)`、トーン切替なし・API なし)、無ければ `FollowupState.NoStoredDraft`(**自動生成はせず**、「今から生成する(AIを1回使用)」ボタンを出す)。旧データ / Worker 未更新でも勝手にトークンを消費しない。Worker `MAX_TOKENS` 2000→2400。**Worker 再デプロイ必要**
