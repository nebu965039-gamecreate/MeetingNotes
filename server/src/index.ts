...
// 2026-09-21: システムプロンプトを圧縮する変更(約3,900字→約3,580字)を試したが、
// 実機での実利用で要約の精度が明確に落ちたとのフィードバックを受けて元の文言に revert 済み。
// 「体言止め・簡潔な記録調」等の細かい言い回しの差が抽出品質に効いている可能性があるため、
// 以後この文字列は不用意に圧縮・言い換えしない(変更する場合は必ず実機で複数件の実利用確認をしてから)。
