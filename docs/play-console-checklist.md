...
- [ ] ガイドに沿って Console で入力。
- [ ] 「D. 未確定・要判断ポイント」の Anthropic データ保持(「一時的処理」の回答)を確定。allowBackup=false・AdMob項目は確定済み。
...
| カスタムアプリアイコン | **済**(2026-08-31、`docs/store-assets/` + アプリのランチャーアイコン差し替え) | — |
| 署名付きリリースビルドの実機動作確認 | エミュレータで署名検証・起動を確認済み(本番鍵) | 本番アップロード鍵での AAB を**実機**で最終確認しておくこと |
| テスター報告の不具合(音声認識 code=5 / 下部ボタン重なり) | 修正済み(0.1.1〜)、**実機再検証待ち** | クローズドテストで再確認 |
