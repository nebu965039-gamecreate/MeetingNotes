...
## クローズドテスト 7回目 (versionName 0.2.0 / versionCode 11)

**2026-09-20 時点でまだ Play Console に提出していない。`app/build.gradle.kts` の `versionCode`/`versionName` は
11 / 0.2.0 に更新済み。** 以前ここに書かれていた案件・パイプラインの下書きは、実際にはパイプラインボードを
試験導入した直後に廃止(フェーズ管理を一本化)する方針変更が入ったため、内容を全面的に書き直した。
「1人CRM」の案件管理・フェーズの管理方法の整理・配色刷新(2026-09-09〜12)に加えて、**アプリ名を
「SecretAlly」に改称**し、そのあとの見やすさ改善・ToDo通知・ドラムロール入力などの変更(2026-09-13〜20)も
まとめて含む、通算では最も変更点の多い版。

### 配信前にやること

- [x] `app/build.gradle.kts` の `versionCode` を `11`、`versionName` を `"0.2.0"` に更新(2026-09-20)
- [ ] アプリアイコンの差し替え(ユーザー作成の新アイコンをアダプティブアイコン・Playストア掲載用512×512に反映)
- [ ] Play Console 側のアプリ名を「SecretAlly」に変更、ストア掲載文・スクリーンショット(現状は旧UI・旧アプリ名のまま)を更新
- [ ] 実機で一通り確認(`docs/test-plan.md` の「実機必須」項目に加え、**2026-09-13〜20 の変更は今回セッション中エミュレータが不安定で実機未確認**。特にドラムロール式の時刻入力・ホーム画面の並び替え・通知画面・分析グラフのx軸ラベル・ダーク表示を優先的に確認)
...
```
アプリ名を「SecretAlly」に変更しました(使い方は同じです)。あわせて「商談」の表記を「打ち合わせ」に統一しました。多くの改善も行っています。

【フェーズ管理を刷新】
・案件のフェーズを「現在の状態」の正としました。商談保存直後に、案件作成/フェーズ更新の確認が出ます
・分かりにくかった「パイプラインボード」は廃止しました

【ToDo・通知】
・「スヌーズ」を廃止し、指定した日時に通知する機能に変更しました
・通知はアプリ内の「通知」画面にも記録されます
・時刻の入力をすべて、指で回して選ぶドラムロール式に統一しました

【見やすさ・分析】
・分析画面のグラフや検索欄などの見やすさを改善
・「フォロー」タブに「今月/今年/全期間」の切り替えを追加

【データ・デザイン】
・案件の見積額・成約額・失注理由等を記録、全データのバックアップ/復元、アプリ全体をネイビー基調の配色に刷新
...
**分析・グラフの見やすさ改善(2026-09-17〜19)**
- 棒グラフをx/y軸表示・白背景+枠・青色に統一(`MonthlyTrend`/`SalesScreen.MonthlyBarChart`)。売上グラフの月ラベルは全12ヶ月ぶんを-45度回転させて表示するよう変更
- 売上グラフの月ラベルが枠外にはみ出す不具合を2段階で修正: 高さを実測値から動的計算 → 左右パディングも実測ラベル幅から動的計算(先頭月のラベルが特に欠けやすかった問題を解消)
- 予定表の検索欄・クライアント一覧の背景色を修正(ページ地をティールに統一した影響で背景に溶けていた箇所を白に)
- ホームの「動いていない案件」「本日の予定」をネイビーのダッシュボードカードに統一
- 分析・売上タブのカード背景(`StatCard`/`CurrencyReportCard`)を白に統一
- 分析「フォロー」タブに、売上タブと同じ「今月/今年/全期間」の期間切り替えを追加

**ホーム画面(2026-09-15〜20)**
- 「本日の予定」をダッシュボードより上に移動
- 保存し忘れの商談メモ(下書き復元カード)をホーム最上部・赤枠+「!」バッジで強調表示するよう変更
- 最上部の見出しを「日付+挨拶」から「日付+曜日を大きく表示」のみに簡素化

**アプリ名・用語の変更(2026-09-19)**
- アプリ名を「商談メモ」から「SecretAlly」に改称(パッケージ名・データベース等の内部識別子は変更なし。Google Play/App Store 等で重複が無いことを確認済み)
- アプリ内の「商談」という表記を「打ち合わせ」に統一(画面見出し・ダイアログ・共有テキストなど48箇所)

**ToDo・通知(2026-09-16〜20)**
- ToDoの「スヌーズ」(一時的に一覧から隠す)を廃止し、**指定した日時に端末通知する方式**に変更(DB v29)
- ToDo通知が発火した際、アプリ内の「通知」画面の履歴にも記録されるように変更
- 通知画面: 「予定されているリマインド」欄を削除して通知履歴のみに簡素化、ヘッダーを他画面と同じネイビーに統一、通知履歴の保存件数の上限を99件に設定
- 時刻の入力(次回打ち合わせ・ToDoの期限/通知日時・予定・案件の成約日など、アプリ内すべての時刻入力)を、ダイヤル/キーボード切り替え式から**ドラムロール(スピナー)式**に統一
