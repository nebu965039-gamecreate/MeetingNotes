...
・議事録エクスポートを「共有する / デバイスに保存」を選んでから形式を選ぶ操作に変更(「メール文面で共有」は廃止)
・エクスポート形式を追加:
  - 議事録ぜんぶ … PDF / Word / Markdown
  - ToDoリストだけ … Excel / CSV(タスク管理ツールへの取り込み用)
  - 次回打ち合わせ … カレンダー(.ics)(日時が決まっている場合のみ選べます)
・要約の待ち時間の表示を分かりやすく改善
・「使い方・ヘルプ」画面をカード+アイコン+色付きの見やすいデザインに刷新
・クライアント追加・フォルダ作成などの「作成」ボタンを青で統一(アーカイブのフォルダ作成も画面上部のアイコンに変更)
・アーカイブに検索(タイトル・要約・文字起こし全文から一致箇所を表示)と並び替え(新しい順 / 古い順 / タイトル順)を追加
・アーカイブ画面のクライアント名を上部の帯で見やすく表示
