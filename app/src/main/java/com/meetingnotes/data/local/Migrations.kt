...
/** v22 → v23: ToDo ボードの定期フォロー スヌーズ。`clients.followBoardSnoozedUntil` を追加。 */
val MIGRATION_22_23 = object : Migration(22, 23) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE clients ADD COLUMN followBoardSnoozedUntil INTEGER")
    }
}

/** v23 → v24: 予定に会議URL・場所を追加。 */
val MIGRATION_23_24 = object : Migration(23, 24) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE schedules ADD COLUMN meetingUrl TEXT")
        db.execSQL("ALTER TABLE schedules ADD COLUMN location TEXT")
    }
}

/** v24 → v25: クライアントに流入経路・紹介元を追加。 */
val MIGRATION_24_25 = object : Migration(24, 25) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE clients ADD COLUMN leadSource TEXT")
        db.execSQL("ALTER TABLE clients ADD COLUMN referredBy TEXT")
    }
}

/** v25 → v26: メール文面テンプレートのテーブルを追加。 */
...
/** v26 → v27: 「前回のおさらい」機能の廃止に伴い `client_briefing` テーブルを削除。 */
val MIGRATION_26_27 = object : Migration(26, 27) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("DROP TABLE IF EXISTS `client_briefing`")
    }
}

/** v27 → v28: `todos.snoozedUntil`(ToDo 一件ごとのスヌーズ再表示日時、epoch millis、nullable)。 */
val MIGRATION_27_28 = object : Migration(27, 28) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `todos` ADD COLUMN `snoozedUntil` INTEGER")
    }
}
...
/**
 * v29 → v30: `user_credits` に1日あたりのAI要約トークン消費量トラッキング用の2列を追加
 * (2026-09-21、`CreditPolicy.DAILY_TOKEN_CAP` 参照)。
 */
val MIGRATION_29_30 = object : Migration(29, 30) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `user_credits` ADD COLUMN `tokensUsedToday` INTEGER NOT NULL DEFAULT 0")
        db.execSQL("ALTER TABLE `user_credits` ADD COLUMN `tokensResetDate` TEXT NOT NULL DEFAULT ''")
    }
}
