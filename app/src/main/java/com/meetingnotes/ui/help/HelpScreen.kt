...
private val helpTopics = listOf(
    ...
    HelpTopic(
        Icons.Filled.Schedule, AccentGreen, "長い打ち合わせのコツ",
        HelpBody.Prose(
            "1回の要約でまとめられるのは約60〜80分ぶんまでです。" +
                "それより長い打ち合わせは、区切りのよいところで一度「停止」して保存し、" +
    ...
    HelpTopic(
        ...
        HelpBody.Bullets(
            listOf(
                ...
                "対面: 端末内で文字起こし。音声は端末の外に出ません。静かな場所・対面の打ち合わせ向け",
    ...
    HelpTopic(
        ...
        HelpBody.Bullets(
            listOf(
                "クライアント: 打ち合わせ相手ごとにまとめる単位",
                "フォルダ: クライアント内で打ち合わせを案件別に分ける",
                "グループ: クライアントどうしをまとめる(例:継続案件 / 新規リード)",
                "フォルダ・グループは画面上部のオレンジ色のフォルダアイコンから作成できます"
            )
        )
    ),
    HelpTopic(
        ...
        HelpBody.Bullets(
            listOf(
                "打ち合わせの詳細画面から「共有する / デバイスに保存」を選びます",
        ...
        Column(
            ...
            Text(
                "不具合の報告時にお使いください。録音の動作記録(時間・文字数・エラー回数)と" +
                    "アプリの異常終了の記録が入っています。音声や打ち合わせの内容は含まれません。" +
                    "「共有」で内容を確認してから送れます。",
                style = MaterialTheme.typography.bodyMedium,
                lineHeight = 24.sp,
                color = MaterialTheme.colorScheme.onSurfaceVariant
